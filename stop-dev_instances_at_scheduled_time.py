...
def lambda_handler(event, context):
    instance_ids = get_instances_by_tag("dev", "running")

    if not instance_ids:
        return {
            "statusCode": 200,
            "body": json.dumps({"message": "No running dev instances found.", "stopped_instances": []})
        }

    try:
        ec2_client.stop_instances(InstanceIds=instance_ids)
        logger.info("Stopped dev instances: %s", instance_ids)
    except ClientError as exc:
